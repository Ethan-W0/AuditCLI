...
public record SnapshotConfig(
    ...
    public static SnapshotConfig fromEnvironment() {
        boolean enabled = readBoolean("auditcli.snapshot.enabled", "AUDITCLI_SNAPSHOT_ENABLED", true);
        Path root = Path.of(readString("auditcli.snapshot.dir", "AUDITCLI_SNAPSHOT_DIR",
                Path.of(System.getProperty("user.home"), ".auditcli", "snapshots").toString()));
        int max = readInt("auditcli.snapshot.max", "AUDITCLI_SNAPSHOT_MAX", 50);
        List<String> excludes = mergeExcludes(readString("auditcli.snapshot.excludes", "AUDITCLI_SNAPSHOT_EXCLUDES", ""));
