 ...
 * 1. 系统属性：{@code auditcli.react.token.budget} / {@code auditcli.react.stagnation.window} /
 *    {@code auditcli.react.hard.max.iterations}
 ...
 * 场景（CI / 自动化批跑）通过 {@code -Dauditcli.react.token.budget=N} 显式启用。
...
public class AgentBudget {
    ...
    public static AgentBudget fromLlmClient(LlmClient llmClient) {
        ...
        // 显式 -Dauditcli.react.token.budget=N 仍可启用硬预算，覆盖默认。
        return new AgentBudget(
                readIntProperty("auditcli.react.token.budget", Integer.MAX_VALUE),
                readIntProperty("auditcli.react.stagnation.window", DEFAULT_STAGNATION_WINDOW),
                readIntProperty("auditcli.react.hard.max.iterations", DEFAULT_HARD_MAX_ITERATIONS)
        );
    }
