package com.auditcli.agent;

import com.auditcli.llm.LlmClient;
import com.auditcli.llm.LlmTraceLogger;
import com.auditcli.context.ContextProfile;
import com.auditcli.context.TokenUsageFormatter;
import com.auditcli.lsp.LspDiagnosticReport;
import com.auditcli.memory.ConversationHistoryCompactor;
import com.auditcli.memory.ExplicitMemoryHints;
import com.auditcli.memory.MemoryManager;
import com.auditcli.prompt.PromptAssembler;
import com.auditcli.prompt.PromptContext;
import com.auditcli.prompt.PromptMode;
import com.auditcli.render.PlainRenderer;
...
import com.auditcli.skill.SkillIndexFormatter;
import com.auditcli.skill.SkillRegistry;
import com.auditcli.util.AnsiStyle;
import com.auditcli.tool.ToolRegistry;
import com.auditcli.tool.ToolRegistry.ToolExecutionResult;
import com.auditcli.tool.ToolRegistry.ToolInvocation;
import com.auditcli.util.TerminalMarkdownRenderer;
import com.auditcli.image.ImageReferenceParser;
...
public class Agent {
    ...
    public String getContextStatus() {
        com.auditcli.context.ContextProfile profile = memoryManager.getContextProfile();
        ...
        for (LlmClient.Message msg : conversationHistory) {
            int t = com.auditcli.memory.TokenBudget.estimateMessagesTokens(java.util.List.of(msg));
            switch (msg.role()) {
    ...
    private int estimateToolsSchemaTokens() {
        try {
            return com.auditcli.memory.MemoryEntry.estimateTokens(
    ...
    private void logRequestContext(String scope, List<LlmClient.Tool> tools) {
        ...
        for (int messageIndex = 0; messageIndex < conversationHistory.size(); messageIndex++) {
            ...
            messages++;
            int tokens = com.auditcli.memory.TokenBudget.estimateMessagesTokens(List.of(msg));
        ...
        if (tools != null && !tools.isEmpty()) {
            try {
                toolsSchemaTokens = com.auditcli.memory.MemoryEntry.estimateTokens(
