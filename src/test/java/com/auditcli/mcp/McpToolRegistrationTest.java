...
class McpToolRegistrationTest {
    ...
    private static void withAuditDir(Path tempDir, ThrowingRunnable body) throws Exception {
        String previous = System.getProperty("auditcli.audit.dir");
        System.setProperty("auditcli.audit.dir", tempDir.resolve("audit").toString());
        try {
            body.run();
        } finally {
            if (previous == null) {
                System.clearProperty("auditcli.audit.dir");
            } else {
                System.setProperty("auditcli.audit.dir", previous);
            }
        }
    }
